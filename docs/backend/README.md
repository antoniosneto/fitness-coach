...
- **Implementado:** Auth (signup, login com JWT, Argon2id, proteção força bruta 5 falhas → 429), Onboarding (PUT profile, PUT goals com validação 1,5%/semana), JWT Guard/Strategy, Prisma 7 com adapter pg, banco local (Docker ou Homebrew), **seed TACO** (alimentos base em FOOD, tenant default), collection Postman com teste 422.
...
- **Pendente (próximos passos):** POST /api/v1/plans/weekly, Motor de Metas (GCT, déficit, TACO, treino), preferências (treino/esporte) se definido no contrato, testes e2e dos SCN-*, recuperação de senha (REQ-AUTH-003).
